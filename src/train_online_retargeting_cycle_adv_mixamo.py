      ...
      for _, batchidx in mini_batches:
        ...
        if len(batchidx) == batch_size:
          ...
          for b in xrange(batch_size):
            ...
            low = 0
            high = trainseq[rnd_idx].shape[0]-max_steps
            if low >= high:
              stidx = 0
            else:
              stidx = np.random.randint(low=low, high=high)

            crealSeq = trainseq[rnd_idx][stidx:stidx+max_steps]
            crealOffset = trainoffset[rnd_idx][stidx:stidx+max_steps]
            ...
            realSeq_batch.append(crealSeq)
            realOffset_batch.append(crealOffset)
          ...
          realSeq_batch = np.array(realSeq_batch).reshape((
              batch_size, max_steps, -1
          ))
          realOffset_batch = np.array(realOffset_batch).reshape((
              batch_size, max_steps, -1
          ))
          realSeq_batch = np.concatenate(
              (realSeq_batch, realOffset_batch), axis=-1
          )
          ...
          dlf, dlr, gl, lc = gru.train(sess, realSeq_batch, realSkel_batch,
                                       seqA_batch, skelA_batch,
                                       seqB_batch, skelB_batch, aeReg_batch,
                                       mask_batch, step)
...
if __name__ == "__main__":
  ...
  parser.add_argument("--beta", type=float, dest="beta",
                      default=1.0, help="Adversarial training modulation")
  ...
  parser.add_argument("--margin", type=float, dest="margin",
                      default=0.3, help="Discriminator/Generator balancing")
  parser.add_argument("--d_arch", type=int, dest="d_arch",
                      default=1, help="Discriminator architecture index")
  ...
  parser.add_argument("--norm_type", type=str, default="batch_norm",
                      dest="norm_type", help="Batch normalization type")
